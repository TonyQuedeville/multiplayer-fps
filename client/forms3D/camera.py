...
class Camera():
    def __init__(self, position=[0, 0, 0], lookAt=[0, 0, -1], up=[0, 1, 0], speed=0.1):
        self.position = position 
        self.move = [0, 0, 0]   # Progression de déplacement
        self.lookAt = lookAt    # Point de regard de la camera en degré
        self.look = lookAt      # Point de regard actuel de la camera en degré
    ...
    def calcul_rotate_camera(self, angle):        
        ...
        if angle > 0:
            signe = -1
        # self.rotate = [np.sin(np.pi/(self.rotate_speed*2))*signe + self.position[0], self.position[1], -np.cos(np.pi/(self.rotate_speed*2)) + self.position[2]]
