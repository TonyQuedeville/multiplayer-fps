...
class Game():
    def __init__(self, players=[]):
        ...
        self.tokens = [
            ...
            [[1,0,1],[11,0,9],[12,0,10],[11,0,7],[10,0,8],[5,0,9],[5,0,7],[11,0,3],[12,0,5],[12,0,8]], # Level2
        ...
        i = 0
        for token_level in self.tokens:
            for token in token_level:
                token.append(i)
                i+=1
    ...
    # Médaillons
    def sup_token(self, token_id):
        result = False
        for token_level in self.tokens:
            for token in token_level:
                if token[3] == token_id:
                    token_level.remove(token)
                    result = True
        return result
    
    def update_tokens(self, id, data):
        player = self.get_player_by_id(id)
        if player:
